...
export async function fetchAIExplanationApi(text: string): Promise<AiResponse> {
  ...
  if (lowerText.includes('infinite loop')) {
    message = "Infinite Loop terjadi bila batas kondisi (seperti i < 10) tidak pernah tercapai, biasanya karena kamu lupa menambahkan increment (i++). Program akan terus berjalan sampai kehabisan memori. Harus sangat berhati-hati!";
  } else if (lowerText.includes('for loop')) {
    message = "FOR Loop ibarat kamu disuruh berlari keliling lapangan sebanyak 5 putaran. Kamu sudah tahu persis jumlah akhirnya (5) sebelum mulai.";
  } else if (lowerText.includes('while loop')) {
    message = "WHILE Loop ibarat kamu disuruh berlari keliling lapangan sampai kamu capek. Kamu tidak tahu berapa putaran pastinya, tapi tahu persis kapan harus berhenti (saat capek).";
  }
