  ...
  return (
    <div className="glass-card p-6 md:p-8 rounded-3xl border-t border-indigo-500/30 bg-indigo-950/20 relative overflow-hidden h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-2 relative z-10">
        <div className="w-10 h-10 bg-indigo-500/20 border border-indigo-500/50 rounded-xl flex items-center justify-center text-indigo-400 text-xl shadow-inner shadow-indigo-500/30">📝</div>
      ...
      <ul className="space-y-3 m-0 p-0 relative z-10 overflow-y-auto flex-1 hide-scrollbar">
        {filteredTasks.length === 0 ? (
          <li className="text-center py-6 text-slate-500 text-sm">Tidak ada tugas dalam kategori ini.</li>
        ) : (
            ...
            >
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="truncate">
                  <span className={`block text-sm font-bold transition-all truncate ${task.completed ? 'line-through text-emerald-200/50' : 'text-slate-200'}`}>
                    {task.title}
                  </span>
                </div>
              </div>
