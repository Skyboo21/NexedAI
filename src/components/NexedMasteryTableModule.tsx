      ...
      {!isLoading && isError && (
        <div className="p-6 bg-pink-950/30 border border-pink-500/30 rounded-2xl text-pink-200 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="text-sm text-pink-200/80 mb-4">{error?.message || "Koneksi terputus."}</div>
            <button
              onClick={() => refetch()}
        ...
        <div className="bg-black/20 rounded-2xl border border-white/5 overflow-hidden">
          {filteredData.length === 0 ? (
            <div className="py-16 px-6 text-center">
              ...
              <div className="text-sm text-slate-500">Coba ubah kata kunci pencarian atau reset filter.</div>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-white/10 bg-white/5 text-slate-400">
                    ...
                    <th className="py-4 px-6 font-semibold uppercase tracking-wider text-xs">Nama Mahasiswa</th>
                    <th className="py-4 px-6 font-semibold uppercase tracking-wider text-xs">Topik Pembelajaran</th>
                    <th className="py-4 px-6 font-semibold uppercase tracking-wider text-xs">Penguasaan</th>
                    <th className="py-4 px-6 font-semibold uppercase tracking-wider text-xs">Status Risiko</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {filteredData.map((row) => {
                    const statusBg = row.status === 'Aman' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 
                                     row.status === 'Perlu Perhatian' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 
                                     'bg-pink-500/10 text-pink-400 border-pink-500/20';
                    const progressColor = row.mastery >= 80 ? 'from-emerald-400 to-emerald-600' : 
                                          row.mastery >= 60 ? 'from-orange-400 to-orange-600' : 
                                          'from-pink-400 to-pink-600';

                    return (
                      <tr key={row.id} className="hover:bg-white/5 transition-colors group">
                        <td className="py-4 px-6 font-medium text-slate-500 group-hover:text-slate-400">#{row.id}</td>
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold text-slate-300 w-10 text-right">{row.mastery}%</span>
                          </div>
                        </td>
                        <td className="py-4 px-6">
                          <span className={`inline-block px-3 py-1 rounded-md text-[11px] font-bold border ${statusBg}`}>
                            {row.status}
                          </span>
                        </td>
                      </tr>
